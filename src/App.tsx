...
export default function AnnouncementApp() {
  ...
  const handleSearch = () => {
    ...
    const filtered = students.filter((s) => 
      s.nama.toUpperCase().includes(nameUpper) &&
      (jalur === "Semua" || s.jalur === jalur)
    );

    setResult(filtered.length > 0 ? filtered[0] : false);
  };

  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-sky-100 to-emerald-100 p-4">
      <img src="/logo-sdn2.png" alt="Logo Sekolah" className="w-28 md:w-32 mb-4" />
      <motion.h1
      ...
      >
        ...
        <option value="Jalur Domisili">Jalur Domisili</option>
      ...
      {result !== null && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="w-full max-w-md mt-4 print:bg-white"
        >
          {result ? (
            <Card className="bg-emerald-50 border-emerald-400">
              <CardContent className="p-6">
                ...
                <div className="mt-4 print:hidden">
                  <Button onClick={() => window.print()}>Unduh Bukti</Button>
                </div>
              </CardContent>
            </Card>
          ) : (
            <Card className="bg-rose-50 border-rose-400">
              <CardContent className="p-6">
                ...
                <p>
                  Nama tidak ditemukan sebagai peserta yang diterima. Coba periksa ejaan atau filter jalur yang dipilih.
                </p>
              </CardContent>
            </Card>
          )}
